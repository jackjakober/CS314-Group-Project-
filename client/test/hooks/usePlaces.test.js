...
describe('usePlaces', () => {
    ...
    test('awohara: appends three places', async () => {
        fetch.mockResponse(REVERSE_GEOCODE_RESPONSE);
        expect(hook.current.places).toEqual([]);

        let placesToAdd = [mockLatLng, mockLatLng, mockLatLng];

        await act(async () => {
            hook.current.placeActions.appendBunch(placesToAdd);
        });

        expect(hook.current.places).toEqual([MOCK_PLACE_RESPONSE,MOCK_PLACE_RESPONSE,MOCK_PLACE_RESPONSE]);
    });
