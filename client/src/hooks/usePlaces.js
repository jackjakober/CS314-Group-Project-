...
export function usePlaces() {
    ...
    const placeActions = {
        ...
        appendBunch: async (places) => appendBunch(places,context),
    ...
    return {places, distances, selectedIndex, placeActions};
}

async function appendBunch(latLngs, context) {
    const { places, setPlaces, setSelectedIndex } = context;
    const newPlaces = [...places];

    for(let place in latLngs)
    {
        const fullPlace = await reverseGeocode(latLngs[place]);
